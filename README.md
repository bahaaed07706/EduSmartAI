...
- **Chatbot is context-grounded, NOT retrieval RAG.** It injects bounded course-material text into the prompt (no query-based retrieval, ranking, or vector store). It requires a valid `GROQ_API_KEY` (`gsk_...`) for LLM answers; without one it returns local, data-grounded fallback responses and never exposes configuration to end users. **Live-provider answer quality is unverified** in this environment (no key available) — it must be checked manually with a real key.
